# 📦 LandGod Downloads

## Packages

| Package | Type | Install On |
|---------|------|-----------|
| `landgod-0.1.1.tgz` | Worker (Node.js) | Managed devices |
| `landgod-gateway-0.1.1.tgz` | Gateway (Node.js) | Agent machine |
| `landgod_gateway_server-0.1.1-py3-none-any.whl` | Gateway (Python) | Agent machine |
| `landgod_gateway-0.1.1-py3-none-any.whl` | Client SDK (Python) | Agent machine |

## Install from GitHub

```bash
# Gateway (Node.js)
npm install -g https://github.com/zhou12138/cli-server/raw/master/downloads/landgod-gateway-0.1.1.tgz

# Gateway (Python)
pip install https://github.com/zhou12138/cli-server/raw/master/downloads/landgod_gateway_server-0.1.1-py3-none-any.whl

# Worker
npm install -g https://github.com/zhou12138/cli-server/raw/master/downloads/landgod-0.1.1.tgz

# Python SDK
pip install https://github.com/zhou12138/cli-server/raw/master/downloads/landgod_gateway-0.1.1-py3-none-any.whl
```

## Quick Start

1. [Install Gateway](./QUICKSTART-GATEWAY.md)
2. [Install Worker](./QUICKSTART-WORKER.md)

## Documentation

See [docs/](../docs/)
