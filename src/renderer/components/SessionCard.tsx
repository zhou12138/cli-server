...
export default function SessionCard({ command, state, timestamp, exitCode, durationMs, clientIp, onClick }: SessionCardProps) {
  const { t } = useI18n();
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <span
          className={`shrink-0 text-xs px-1.5 py-0.5 rounded font-medium ${isRunning
        ...
        >
          {isRunning
            ...
            : exitCode === null
              ? t('dashboard.killed')
              : t('dashboard.exit', { code: exitCode })}
        </span>
      </div>
